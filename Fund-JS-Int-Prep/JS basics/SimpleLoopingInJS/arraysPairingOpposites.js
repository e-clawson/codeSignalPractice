...
1 to 500 and is always an odd number. The elements of the array span values from 
−10^6 to 10^6 inclusive. 
The goal is to return a new array constructed by traversing the initial array in a specific order, 

outlined as follows:

- Begin with the middle element of the array.
- For each subsequent pair of elements, alternate between taking two elements 
from the left and two elements from the right, relative to the middle. 
Take those two elements and place it to the back of the newly constructed array 
preserving the order of the elements in the pair.
...
function unusualTraversal(array) {
    ...
    while (left >= 0 || right < length) {
        if (left >= 1) {
            newArr.push(array[left - 1], array[left]);
            left -= 2;
        } else if (left === 0) {
            newArr.push(array[left]);
            left--;
        } if (right < length - 1) {
            newArr.push(array[right], array[right + 1]);
            right += 2;
